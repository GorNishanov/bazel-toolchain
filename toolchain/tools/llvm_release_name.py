...
def _ubuntu_osname(arch, version, major_llvm_version, llvm_version):
    ...
    is_llvm_major_release = (_minor_llvm_version(llvm_version) == 0) and (
        _patch_llvm_version(llvm_version) == 0)
    ...
    if (major_ubuntu_version >= 20 and (not version.startswith("20.04"))
        ...
        if major_llvm_version >= 15:
            ...
            os_name = "linux-gnu-ubuntu-" + \
                ("20.04" if major_ubuntu_version >= 20 else "16.04")
        ...
        if llvm_version in ["15.0.2"]:
            ...
            sys.exit("LLVM patch release %s not available for Ubuntu %s" %
...
def _linux(llvm_version, distname, version, arch):
    ...
    if arch in ["aarch64", "armv7a", "mips", "mipsel"]:
        ...
        os_name = _ubuntu_osname(
            arch, version, major_llvm_version, llvm_version)
    elif ((distname in ["linuxmint"]) and (version.startswith("21") or version.startswith("20") or version.startswith("19"))):
        os_name = _ubuntu_osname(
            arch, "20.04", major_llvm_version, llvm_version)
        ...
        if 8 <= float(version) < 9:
            os_name = _ubuntu_osname(
                arch, "18.04", major_llvm_version, llvm_version)
        elif float(version) >= 9:
            os_name = _ubuntu_osname(
                arch, "20.04", major_llvm_version, llvm_version)
    else:
        sys.exit("Unsupported linux distribution and version: %s, %s" %
                 (distname, version))
...
def _resolve_version_for_suse(major_llvm_version, llvm_version):
    ...
    if major_llvm_version < 10:
        ...
        os_name = _ubuntu_osname(
            "x86_64", "20.04", major_llvm_version, llvm_version)
    return os_name
